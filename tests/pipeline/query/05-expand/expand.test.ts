...
import type { RerankedHit } from '../../../../src/pipeline/query/04-rerank/rerank.ts';
import {
  buildSymbolGraph,
  expandResults,
} from '../../../../src/pipeline/query/05-expand/expand.ts';
...
describe('expandResults', () => {
  ...
  it('does not let a hit with many callers crowd out its callees (regression)', () => {
    // 'shared' has 5 callers AND 1 callee — concatenating callers-then-callees
    // before slicing to maxPerHit used to mean the 5 callers alone filled the
    // cap and the callee was never added, contradicting expandResults' own
    // "callers AND callees" doc comment.
    const target = chunk({
      id: 'target',
      symbolName: 'shared',
      content: 'function shared() { helper(); }',
    });
    const helper = chunk({ id: 'helper', symbolName: 'helper', content: 'function helper() {}' });
    const callers = Array.from({ length: 5 }, (_, i) =>
      chunk({
        id: `caller${i}`,
        symbolName: `caller${i}`,
    ...
    const expanded = expandResults([target, helper, ...callers], reranked, {
      maxPerHit: 3,
      maxTotal: 100,
    });

    expect(expanded.some((h) => h.id === 'helper' && h.via === 'callee')).toBe(true);
    expect(expanded.filter((h) => h.via === 'caller').length).toBeGreaterThan(0);
  });
